...
@section('title', 'Danh sách các đội thi đã xóa ')
    ...
    <div class="card card-flush p-4">
        ...
        <div class="table">
            <table class="table table-responsive">
                ...
                <tbody>
                    @if (count($listTeamSofts) > 0)
                        @foreach ($listTeamSofts as $key => $listTeamSoft)
                            <tr>
                                <td>
                                    {{ (request()->has('page') && request('page') !== 1 ? $listTeamSofts->perPage() * (request('page') - 1) : 0) + $key + 1 }}
                                </td>
                                <td> <button style="border:none;background:#ffff" type="button" data-bs-toggle="modal"
                                        data-bs-target="#deltai_team_{{ $listTeamSoft->id }}">
                                        {{ $listTeamSoft->name }}
                                    </button>

                                    <!-- Modal -->
                                    <div style="margin-left:600px;width:800px" class="modal fade "
                                        id="deltai_team_{{ $listTeamSoft->id }}" tabindex="-1"
                                                    ...
                                                    <div class="row">
                                                        <div class="col md-6">
                                                            <img style="width:200px;height:200px"
                                                                src="{{ Storage::disk('s3')->has($listTeamSoft->image) ? Storage::disk('s3')->temporaryUrl($listTeamSoft->image, now()->addMinutes(5)) : 'https://skillz4kidzmartialarts.com/wp-content/uploads/2017/04/default-image.jpg' }}"
                                                                alt="">
                                                            <h4 class="mt-3">Ngày Tạo :
                                                                {{ $listTeamSoft->created_at }}
                                                            </h4>
                                                            <h5>Tham gia cuộc thi
                                                                :{{ $listTeamSoft->contest->name ?? 'Chưa có cuộc thi ' }}
                                                            </h5>
                                                        </div>
                                                        <div class="col md-6">
                                                            <h3>Tên Đội : {{ $listTeamSoft->name }}</h3>
                                                            <h4 class="mt-3"> Danh sách thành viên </h4>
                                                            <ul>
                                                                @if (count($listTeamSoft->members) > 0)
                                                                    @foreach ($listTeamSoft->members as $user)
                                ...
                                <td>{{ $listTeamSoft->contest->name ?? 'Chưa có cuộc thi ' }}</td>
                                <td>{{ $listTeamSoft->deleted_at }}
                                    <p>{{ \Carbon\Carbon::parse($listTeamSoft->deleted_at)->diffForHumans() }}</p>
                                </td>
                                <td>
                                    <div class="btn-group dropstart">
                                        ...
                                        <ul class="dropdown-menu px-4 ">
                                            ...
                                            <li class="my-3">
                                                <a
                                                    href="{{ route('admin.team.soft.backup', ['id' => $listTeamSoft->id]) }}">
            ...
            {{ $listTeamSofts->appends(request()->all())->links('pagination::bootstrap-4') }}
        </div>
...
@section('page-script')
    <script>
        let url = '/admin/teams/team-soft-delete?team_soft_delete=1&keyword=';
