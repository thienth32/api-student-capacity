...
@section('content')
    ...
    <div class="card card-flush p-4">
        <div class="row">
            <div class=" col-lg-6">
                <div class=" d-flex justify-content-start">
                    ...
                    <a class="ms-3" href="{{ route('admin.branch.list.soft.deletes') }}">
            ...
            <div class=" col-lg-6">
                ...
                <div class=" d-flex flex-row-reverse bd-highlight">
                    <a href="{{ route('admin.branch.create') }}" class=" btn btn-primary">Tạo mới cơ sở</a>
                </div>
                @endrole
            </div>
        </div>
        ...
        <div class="table-responsive table-responsive-md">
            <table class="table table-row-bordered table-row-gray-300 gy-7  table-hover ">
                <thead>
                    <tr>
                        ...
                        <th scope="col"> Tổng số tài khoản
                            <span role="button" data-key="users_count" data-bs-toggle="tooltip"
                                title="Sắp xếp theo số tài khoản"
                                class=" svg-icon svg-icon-primary  svg-icon-2x format-database">
                                <!--begin::Svg Icon | path:/var/www/preview.keenthemes.com/metronic/releases/2021-05-14-112058/theme/html/demo2/dist/../src/media/svg/icons/Navigation/Up-down.svg--><svg
                                    xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                    style="width: 14px !important ; height: 14px !important" width="24px"
                            ...
                            {{-- @if (request()->has('sort'))
                                <th scope="row">
                                    @if (request('sort') == 'desc')
                                        {{ (request()->has('page') && request('page') !== 1 ? $branches->perPage() * (request('page') - 1) : 0) + $key + 1 }}
                                    @else
                                        {{ request()->has('page') && request('page') !== 1 ? $total - $branches->perPage() * (request('page') - 1) - $key : ($total -= $key == 0 ? 0 : 1) }}
                                    @endif
                                </th>
                            @else
                                <th scope="row">
                            ...
                            <td>
                                ...
                                <div data-bs-toggle="tooltip" title="Thao tác" class="btn-group dropstart">
                                    ...
                                    <ul class="dropdown-menu ps-5   ">
                                        <li class="py-3">
                                            <a href="{{ route('admin.branch.edit', ['branch_id' => $branch->id]) }}">
                                        ...
                                        <li class="py-3">
                                                <form action="{{ route('admin.branch.destroy', ['branch_id' => $branch->id]) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('delete')
            ...
            {{ $branches->appends(request()->all())->links('pagination::bootstrap-4') }}
        </div>
    </div>
