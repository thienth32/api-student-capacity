...
var KTChartsWidget18 = function () {
    var chart;
    // Private methods
    var initChart = async function(chartData) {
        ...
        const listRegisters = chartData.data.map(item => {
            return item.teams.length;
        });
        const listContestNames = chartData.data.map(item => {
            return item.name;
        });
        var options = {
            series: [{
                name: 'Danh sách đội đăng ký',
                data: listRegisters
            }],
            chart: {
                fontFamily: 'inherit',
